...
db.sales.find({ $and: [{ item: "abc" }, { quantity: { $lt: 10 } }] });

db.sales.find({
	$or: [{ item: "abc" }, { item: "def" }],
});

db.sales.updateMany({ quantity: { $lte: 5 } }, { $inc: { price: 10 } });

db.sales.find({ item: "jkl" });

db.sales.updateOne({ item: "jkl" }, { $set: { item: "lmn" } });

db.sales.updateMany({ item: "abc" }, { $set: { dummyField: "Dummy" } });

db.sales.updateMany({ quantity: { $gte: 10 } }, { $mul: { price: 0.9 } });

db.sales.insert({ item: "xyz", discounts: ["hdfc", "icici"] });

db.sales.updateOne({ item: "abc" }, { $push: { discounts: "axis" } });
db.sales.updateOne(
	{ discounts: { $exists: true } },
	{ $addToSet: { discounts: "axis" } }
);
